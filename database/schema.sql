...
-- ============================================================================
-- SECTION 1: ROLES AND USER MANAGEMENT
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS public.user_profiles (
  ...
  kelas VARCHAR,                    -- Class name (e.g., "7A", "8B")
...
COMMENT ON COLUMN public.user_profiles.userid IS 'Primary key, references auth.users';
...
-- ============================================================================
-- Note: These policies should be customized based on your security requirements
-- The examples below provide a starting point

-- Enable RLS on tables
ALTER TABLE public.user_profiles ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.aktivitas ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.aktivitas_field_values ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.aktivitas_field_images ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.komentar ENABLE ROW LEVEL SECURITY;

-- Policy: Students can view their own profile
CREATE POLICY student_view_own_profile ON public.user_profiles
    FOR SELECT
    USING (auth.uid() = userid);

-- Policy: Students can view their own activities
CREATE POLICY student_view_own_activities ON public.aktivitas
    FOR SELECT
    USING (auth.uid() = userid);

-- Policy: Students can insert their own activities
CREATE POLICY student_insert_own_activities ON public.aktivitas
    FOR INSERT
    WITH CHECK (auth.uid() = userid);

-- Policy: Students can view their own field values
CREATE POLICY student_view_own_field_values ON public.aktivitas_field_values
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM public.aktivitas
            WHERE aktivitas.activityid = aktivitas_field_values.activityid
            AND aktivitas.userid = auth.uid()
        )
    );

-- Policy: Students can insert their own field values
CREATE POLICY student_insert_own_field_values ON public.aktivitas_field_values
    FOR INSERT
    WITH CHECK (
        EXISTS (
            SELECT 1 FROM public.aktivitas
...
-- Policy: Students can view comments on their activities
CREATE POLICY student_view_comments ON public.komentar
    FOR SELECT
    USING (auth.uid() = siswaid);

-- Note: Additional policies for teachers, parents, and admins should be added
-- based on specific requirements. See BACKEND.md for authorization patterns.
...
-- Example: Create sample fields (uncomment if needed)
/*
-- Get category IDs first, then insert fields
INSERT INTO public.category_fields (categoryid, field_key, label, type, required, order_index)
SELECT 
  categoryid,
  'prayer_time',
  'Prayer Time',
...
WHERE categoryname = 'Prayer Details';
*/
...
GRANT ALL ON public.kegiatan TO authenticated;
GRANT ALL ON public.category TO authenticated;
GRANT ALL ON public.category_fields TO authenticated;
GRANT ALL ON public.kegiatan_categories TO authenticated;
GRANT ALL ON public.aktivitas TO authenticated;
GRANT ALL ON public.aktivitas_field_values TO authenticated;
GRANT ALL ON public.aktivitas_field_images TO authenticated;
GRANT ALL ON public.komentar TO authenticated;
GRANT SELECT ON public.submission_window TO authenticated;
...
-- - 11 core tables for the G7 KAIH system
...
--
-- Next Steps:
-- 1. Review and customize RLS policies based on your security requirements
-- 2. Add any additional indexes based on query patterns
-- 3. Set up initial data (roles, categories, fields) as needed
-- 4. Test the schema with sample data
-- 5. Run migrations in your Supabase instance
--
